...
重み付きで何かを順番に選びたい人向けの話です。ロードバランサの文脈で有名なアルゴリズムですが、実際に効いたのは投稿スケジューラでした。
...
これが致命的だったのは、以前 SNS アカウントを 1 つ止められた経験があったからです。1 日の投稿数を増やしすぎたのが原因でした。頻度を落として運用を再開したのに、今度は「同じ話題の連投」で同じ轍を踏みかけていた。
